...
//Class --> AddToCartPageTest, EndToEndTest,  Test Case--> addToCartTest, endToEndTest	
	...
	@DataProvider(name = "DirectnewAcountDetailsData")
	public Object[][] accountCreationAfterAddToCart() {

		// Totals rows count
		int rows = obj.getRowCount("AccountCreationDataDirect");
		// Total Columns
		int column = obj.getColumnCount("AccountCreationDataDirect");
		int actRows = rows - 1;
		//Created an object of array to store data
		Object[][] data = new Object[actRows][1];
		...
		for (int i = 0; i < actRows; i++) {
			Map<Object, Object> hashMap = new HashMap<>();
			for (int j = 0; j < column; j++) {
				hashMap.put(obj.getCellData("AccountCreationDataDirect", j, 1),
						obj.getCellData("AccountCreationDataDirect", j, i + 2));
